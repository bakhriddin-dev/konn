...
router.get('/:username/public', async (req, res) => {
  try {
    ...
    const user = await User.findOne({ username }).select('-__v -createdAt -updatedAt -email');
...
router.put('/me', auth, async (req, res) => {
  try {
    const updates = (({ name, bio, username, theme, avatar }) => ({ name, bio, username, theme, avatar }))(req.body);
    const user = await User.findById(req.user.id);
    if (!user) return res.status(404).json({ message: 'User not found' });
...
router.post('/me/links', auth, async (req, res) => {
  try {
    ...
    const user = await User.findById(req.user.id);
    user.links.push({ title, url, icon: icon || '', enabled: typeof enabled === 'boolean' ? enabled : true });
    user.recalculateMetrics();
    await user.save();
...
router.delete('/me/links/:linkId', auth, async (req, res) => {
  try {
    ...
    if (user.links.length === initialLen) return res.status(404).json({ message: 'Link not found' });
...
router.post('/:username/click', async (req, res) => {
  try {
    ...
    res.json({ message: 'click recorded', totalClicks: user.totalClicks, clickRetention: user.clickRetention });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
  }
